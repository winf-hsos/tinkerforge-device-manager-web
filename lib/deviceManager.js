...
import { MotionDetectorWrapper } from './wrapper/MotionDetectorWrapper.js';
import { MotorizedLineraPotiWrapper } from './wrapper/MotorizedLinearPotiWrapper.js';
...
deviceManager.setLogFunction = function (logFunction) {
    _logFunction = logFunction;
}

function _log(message) {
    console.log(message);
    if (typeof _logFunction !== "undefined")
        _logFunction(message);

}

deviceManager.log = _log;

deviceManager.add = function (uid, deviceIdentifier) {
    var device = _wrapDevice(uid, deviceIdentifier);
    if (typeof device !== "undefined") {
        let message = "Found device >" + device.getName() + "< (UID: " + device.getUID() + "/ DID: " + device.getDeviceIdentifier() + ")";
        _log(message);
...
deviceManager.addAgain = function (uid, deviceIdentifier) {
    var device = _wrapDevice(uid, deviceIdentifier);
    if (typeof device !== "undefined") {
        let message = "Re-connected device >" + device.getName() + "< (UID: " + device.getUID() + "/ DID: " + device.getDeviceIdentifier() + ")";
        _log(message);
        _devices.set(uid, device);
    }
    return device;
...
deviceManager.remove = function (uid) {
    ...
    if (_devices.has(uid)) {
        ...
        let message = "Lost device >" + device.getName() + "< (UID: " + device.getUID() + "/ DID: " + device.getDeviceIdentifier() + ")";
        _log(message);
        _devices.delete(uid);
    }
}
...
function _wrapDevice(uid, deviceIdentifier) {
    ...
    switch (deviceIdentifier) {
        ...
        case 233:
            deviceName = Tinkerforge.BrickletMotionDetector.DEVICE_DISPLAY_NAME;
            device = new Tinkerforge.BrickletMotionDetector(uid, _ipcon);
            deviceWrapper = new MotionDetectorWrapper(device, uid, deviceIdentifier, deviceName);
            break;
        ...
        case 267:
            deviceName = "Motorized Linear Poti Bricket";
            device = new Tinkerforge.BrickletMotorizedLinearPoti(uid, _ipcon);
            deviceWrapper = new MotorizedLineraPotiWrapper(device, uid, deviceIdentifier, deviceName);
            break;
        ...
        default:
            ...
            _log("Device with device ID >" + deviceIdentifier + "< is not supported yet.");
            break;
    }
